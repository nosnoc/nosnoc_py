...
class NosnocCustomScholtesFBSolver(NosnocSolverBase):
    ...
    def __init__(self, opts: NosnocOpts, model: NosnocModel, ocp: Optional[NosnocOcp] = None):
        ...
        for i in range(nG):
            kkt_comp = ca.vertcat(kkt_comp, self.G[i] + mu_G[i] - ca.sqrt(self.G[i] ** 2 + mu_G[i] ** 2  + tau**2))
        for i in range(n_comp):
            kkt_comp = ca.vertcat(kkt_comp, slack[i] + mu_s[i] - ca.sqrt(slack[i] ** 2 + mu_s[i] ** 2 + tau**2))
        ...
        # hessian without mu * G
        stationarity_w_no_muG = ca.jacobian(prob.cost, prob.w).T \
            + ca.jacobian(H, prob.w).T @ lam_H \
            + ca.jacobian(slacked_complementarity, prob.w).T @ lam_comp \
            - ca.jacobian(self.G, prob.w).T @ mu_G
            # - ca.jacobian(slack, prob.w).T @ mu_s
        kkt_eq_jac[:self.nw, :] = ca.jacobian(stationarity_w_no_muG, w_pd)
...
# x2 = D^-1 (y2 - C x_1)
    ...
    def solve(self) -> dict:
        ...
        if opts.print_level == 1:
            print(f"sigma\t\t iter \t res \t min_steps \t min_mu \t max mu \t minG \t cond(A)")
        ...
        for ii in range(opts.max_iter_homotopy):
            ...
            if opts.print_level > 1:
                ...
                print("alpha \t step norm \t kkt res \t min mu \t min G")
            t = time.time()
            alpha_min_counter = 0
            ...
            for gn_iter in range(max_gn_iter):
                ...
                for i in range(n_mu):
                    newton_matrix[-n_mu + i, -n_mu + i] += 1e-5 * np.sign(newton_matrix[-n_mu + i, -n_mu + i])
                # newton_matrix[:self.nw, :self.nw] += 1e-5 * scipy.sparse.eye(self.nw)
                # for i in range(self.nw):
                #     newton_matrix[i, i] += 1e-5 * np.sign(newton_matrix[i, i])
            ...
            if opts.print_level > 1:
                ...
                np_mat = newton_matrix.toarray()
                condA = np.linalg.cond(np_mat)
                print(f"{sigma_k:.2e} \t {gn_iter} \t {nlp_res:.2e} \t {alpha_min_counter}\t {min_mu:.2e} \t {max_mu:.2e}\t {np.min(G_val):.2e} \t{condA:.2e}\t")
                if condA > 1e5:
                    self.check_newton_matrix(np_mat)
                    breakpoint()
