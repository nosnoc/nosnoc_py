...
from .nosnoc_types import RKScheme


class AbstractRKRepresentation(ABC):
    ...
    def _get_order(self):
        if self.rk_scheme == RKScheme.RADAU_IIA:
            order = 2 * self.n_s - 1
        elif self.rk_scheme == RKScheme.GAUSS_LEGENDRE:
            order = 2 * self.n_s
        else:
    ...
    def _generate_butcher_tableau(self, n_s, irk_scheme):
        if irk_scheme == RKScheme.RADAU_IIA:
            if n_s == 1:
                ...
                raise NotImplementedError()
        elif irk_scheme == RKScheme.GAUSS_LEGENDRE:
            if n_s == 1:
                A = np.array([[
                    0.500000000000000,
                ]])
...
class LiftedDifferentialRKRepresentation(DifferentialRKRepresentation):
    def __init__(self, n_s: int, rk_scheme: RKScheme):
        super().__init__(n_s, rk_scheme)
