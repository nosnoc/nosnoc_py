...
def get_default_options(min_h_fraction, **kwargs):
    default_args = {
        ...
        # FE_FE is Patel's Eq. (21)-(22): the sum of the contact forces over a finite element
        # complemented with the sum of the gaps over that same element, which is what makes the
        # contact mode constant within the element. FE_STAGE would be the Baumrucker & Biegler
        # variant he quotes as Eq. (19)-(20).
        "cross_comp_mode": nosnoc.CrossComplementarityMode.STAGE_STAGE,
        "no_initial_impacts": True,
        # gamma_h = 1 leaves the box lower bound at zero, so `min_h_fraction` alone sets the floor
        # on the element length. The eta based step equilibration keeps the non-switching elements
        # equidistant while leaving the two switching boundaries (touchdown and lift off) free, so
        # the contact element can collapse onto the floor.
        #"gamma_h": 1.0,
        #"min_h_fraction": min_h_fraction,
        "step_equilibration": nosnoc.StepEquilibrationMode.L2_RELAXED_SCALED,
        "rho_h": 0.0, # this should disable step equilibration
...
def print_h_table(results):
    ...
    """
    print(f"h_0 = {H0:.5e} s, upper bound (1+gamma_h)*h_0 = {(1+1.0)*H0:.5e} s")
