...
def solve_particle(opts=None, plot=True, friction=False, x0=np.array([0,1,3,0]), Nsim=20, Tsim=2.5):
    ...
    if plot:
        plot_results(results, opts, friction)

    return results


def plot_results(results, opts, friction):
    X_sim = np.array(results['X_sim'])
    alpha_sim = np.array(results['alpha_sim'])
    ...
    # Plot Multipliers
    # plt.figure()
    # plt.plot(t[1:], alpha_sim[:,:,0])
    # plt.plot(t[1:], alpha_sim[:,:,1])
    # if friction:
    #     plt.plot(t[1:], alpha_sim[:,2])
    # # TODO: plot theta 
