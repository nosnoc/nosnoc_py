...
def find_nonscalar(g,w,p=None):
    ...
    ind_g_fun = ca.Function('ind_G', [w,p], [ca.jacobian(g, w)])
    p_val = Sym(np.ones(p.size(1)));
    ...
    if isinstance(w, ca.SX):
        sp = ca.DM(ind_g_fun(w,p) == 1).sparsity();
    else:
    ...
    #ind_nonscalar = setdiff(1:length(g),ind_scalar).T
