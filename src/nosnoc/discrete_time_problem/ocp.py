...
class Ocp(ABC,NLP):
    r"""
    Base class for smooth OCPs generated by nosnoc.
    """
    def __init__(self, dcs, opts):
        ...
        self.opts = opts
        if opts.use_fesd == True: # TODO(@anton) warn
            opts.use_fesd = False
    ...
    def _get_fe_h(self, ii, jj):
        return self.p.T[()].val/(self.opts.N_stages*self.opts.N_finite_elements[ii-1])
